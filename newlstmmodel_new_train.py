...
def train_Model(num_of_Jenres,classes_File):
    ...
    train_batch_size = 1  # if not work decrease to 4 or 8
    ...
    for ep in range(epochs):
        with tqdm(train_loader, unit="batch",position=1,leave=False) as train_epoch:
            ...
            for hp_data, label in train_epoch:
                ...
                if data_cnt % train_batch_size == 0 and data_cnt != 0: #if not work maybe changed batch size to 1
            ...
            if((ep+1) % 5 ==0): #because 0<=0<=34, so we want last epoch to be count
                ...
                with torch.no_grad():
                    ...
                    tepoch.set_postfix(curr_accuracy=((accurcy / len(test_loader)) * 100),theBest_acc=best_accuracy,theEpochNumof_best_acc=epoch_num_of_best_acc)
                    tepoch.update(1)
